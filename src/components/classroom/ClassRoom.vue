      ...
      <v-flex class="pa-2" xs12>
        <v-card color="blue lighten-5">
          <v-carousel class="pa-2">
            <v-carousel-item v-for="(item,i) in mishimaimages" :key="i" :src="item.src"></v-carousel-item>
          </v-carousel>
          <v-card-title primary-title>
            <div>
              <h3 class="headline mb-0">三島教室</h3>
              <div>ああああああああああああああああああああああああああああああああああああああああああああ</div>
            </div>
          </v-card-title>
        </v-card>
        <v-flex class="pa-2" xs12>
          <div class="ggmap" v-html="mishimaMap"></div>
        </v-flex>
      </v-flex>
      <v-flex class="pa-2" xs12>
        <v-card color="blue lighten-5">
          <v-carousel class="pa-2">
            <v-carousel-item v-for="(item,i) in shimizuImages" :key="i" :src="item.src"></v-carousel-item>
          </v-carousel>
          <v-card-title primary-title>
            <div>
              <h3 class="headline mb-0">清水教室</h3>
              <div>ああああああああああああああああああああああああああああああああああああああああああああ</div>
            </div>
          </v-card-title>
        </v-card>
